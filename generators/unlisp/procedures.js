...
goog.provide('Blockly.UnLisp.procedures')
...
Blockly.UnLisp['procedures_defreturn'] = function (block) {
  ...
  var loopTrap = ''
  if (Blockly.UnLisp.INFINITE_LOOP_TRAP) {
    loopTrap = Blockly.UnLisp.prefixLines(
      Blockly.UnLisp.injectId(Blockly.UnLisp.INFINITE_LOOP_TRAP, block),
      Blockly.UnLisp.INDENT)
  }
  ...
  if (returnValue) {
    returnValue = Blockly.UnLisp.INDENT + returnValue + '\n'
  } else if (!branch) {
    branch = ''
  }
  var args = []
  ...
  var body = (xfix1 + loopTrap + branch + xfix2 + returnValue) || '()'
...
Blockly.UnLisp['procedures_callreturn'] = function (block) {
  ...
  var funcName = Blockly.UnLisp.variableDB_.getName(
    block.getFieldValue('NAME'), Blockly.Procedures.NAME_TYPE);
  var args = [];
  for (var i = 0; i < block.arguments_.length; i++) {
    args[i] = Blockly.UnLisp.valueToCode(block, 'ARG' + i,
      Blockly.UnLisp.ORDER_NONE) || 'nil';
  }
